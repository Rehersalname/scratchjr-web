import {createWebStore} from './webStore.js';

const DEBUG = false;
const DEBUG_FILEIO = false;
const DEBUG_RESOURCEIO = false;
const DEBUG_NYI = false;
const DEBUG_DATABASE = false;
const DEBUG_CAMERA = false;
const DEBUG_AUDIO = false;
const DEBUG_AUDIOMETER = false;
...
debugLog('ScratchJr web client ready');
...
class ElectronDesktopInterface {
    ...
    database_stmt(json) {
        return store.getDatabaseManager().stmt(json);

    }
    database_query(json) {
        if (DEBUG_DATABASE) debugLog('beginning database_query', json);
        let res = JSON.stringify(store.getDatabaseManager().query(json));
    ...
    io_getsettings(){
        ...
        let settings = 'scratchjr-web,false,YES,YES';
        return settings;
    ...
    io_getmedia(file){
        ...
        return store.readProjectFileAsBase64EncodedString(file);
    ...
    io_getmediadata(key, offset, length){
        ...
        const mediaString = store.getCachedMedia(key);
        if (mediaString) {
    ...
    io_getmediadone(key){
        ...
        store.removeFromMediaCache(key);
        return true;
    ...
    io_getmedialen(file, key){
        ...
        const encodedStr = store.readProjectFileAsBase64EncodedString(file);
        store.cacheMedia(key, encodedStr);
    ...
    io_setmedia(str,  ext){
        ...
        const filename = `${store.getMD5(str)}.${ext}`;
        store.writeProjectFile(filename, str);
        return filename;
    ...
    io_getmd5(str){
        ...
        return (str) ? store.getMD5(str) : null;
    }


    io_remove(str){
        if (DEBUG_NYI)  debugLog('io_remove - NYI', str);
        store.removeProjectFile(str);
        return true;
    ...
    io_cleanassets(str){
        ...
        store.getDatabaseManager().cleanProjectFiles(str);
        return true;
    ...
    io_registersound(dir, name){
        ...
        if (!this.currentAudio[name]) {
            let dataUri = store.getAudioDataUri(name);
...
createWebStore().then((webStore) => {
    store = webStore;
    window.tablet = new ElectronDesktopInterface();
}).catch((err) => {
    console.error('Failed to start ScratchJr web storage', err);
});
